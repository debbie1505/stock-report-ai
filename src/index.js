  ...
  try {
    const stockData = await Promise.all(
      tickersArr.map(async (ticker) => {
        const url = `https://polygon-api-worker.omoyataiwo21.workers.dev/?ticker=${ticker}&startDate=${dates.startDate}&endDate=${dates.endDate}`;
        const response = await fetch(url);
        const data = await response.text();
        const status = await response.status;
        //checks if the status is 200 or ok
        if (status === 200) {
          apiMessage.innerText = "Creating report...";
          return data;
          //indicates an error from the worker
        } else {
          throw new Error("Worker error; " + data);
        }

        /* //Another way of doing this -- more straight forward approach 
